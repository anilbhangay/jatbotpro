...
const Grammar  = () => {
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText(grammerCheck)
      .then(() => {
  ...
  const parseGrammarCheck = (text) => {
    ...
    const sentences = trimmedText.split('.'); // Assuming sentences end with '.'

    // Apply grammar check and color formatting to each sentence
    const correctedSentences = sentences.map((sentence) => {
      // Apply your grammar check logic here, and mark the sentence if it needs correction
      const needsCorrection = false; // Example logic, replace with your actual logic
  
      // If the sentence needs correction, wrap it in a span with red color
      if (needsCorrection) {
        return `<span style="color: red;">${sentence}</span>`;
      } else {
        // Return the original sentence without color formatting
        return sentence;
      }
    });

    // Join the corrected sentences back into a single string
    let formattedText = correctedSentences.join('. '); // Join sentences with a period and space

    // Replace the HTML span elements with the text color set to red with the actual text
    formattedText = formattedText.replace(/<span style="color: red;">(.*?)<\/span>/g, '<font color="red">$1</font>');
  
};

    
    // Join the corrected sentences back into a single strin
  ...
  return (
    ...
    <div className='container-main-gram'>
      ...
      <div>
          ...
          <div><button className='gram-button'onClick={handleUpload}>GRAMMAR CHECK</button></div>
      <div className='trash-gram'onClick={handleDelete}><FontAwesomeIcon icon={faTrash} /></div>
      </div>
          ...
          <div className='box-left-data-gram'>
            <div className='upload-box-gram'>
                ...
                <div onClick={() => setShowTextarea(true)}>
                  {showTextarea ? null : (!file ? <p className="text-label-trans">PASTE TEXT<FontAwesomeIcon icon={faClipboard} /></p> : null)}
                </div>
            </div>
            <div className='box-left-text-gram'>
            {!showTextarea && file && (
                <div className='upload-text-gram'>
                  <textarea value={original_text} onChange={e => setText(e.target.value)} id='text-margin-gram' />
                </div>
              )}
              {showTextarea && (
                <div className='text-area-gram' >
                  <textarea value={inputText}  onChange={e => setInputText(e.target.value)} placeholder='enter a text' id='text-area-data-gram'></textarea>
                </div>
              )}
            </div> 
            </div>
        <div className='box-right-gram'><div className='box-right-text-gram' >
        {grammerCheck && (
        <div>
        <textarea  value={grammerCheck} color='Red' id='textarea-right-gram'>
         
        </textarea>
        <div className='text-gram-copy-download'>
          <span onClick={handleCopy} onMouseEnter={handleCopyIconHover} onMouseLeave={handleCopyIconLeave}>
            <FontAwesomeIcon id='text-gram-copy' icon={faCopy} />
          </span>
          <FontAwesomeIcon id='text-gram-download' icon={faDownload} onClick={handleDownload} />
