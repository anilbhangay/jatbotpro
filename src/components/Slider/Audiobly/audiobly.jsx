...
import { faTrash, faCloudUploadAlt,faMicrophone,faMicrophoneSlash} from "@fortawesome/free-solid-svg-icons";
    ...
    <div className='container-audio'>
      <div className='container-main-audio'>
        <div className="head-top-audio">
          <div>
            <select  className='languages-audio' onChange={handleLanguageChange1} value={targetLanguage1}> 
          ...
          <div className='audio-button'><button onClick={handleUpload}>TRANSLATOR</button></div>
          <div ><select className='languages-audio' onChange={handleLanguageChange2} value={targetLanguage2}>
          ...
          <div className='mics-icon-audibly'>
            ...
            <div>
              <audio ref={audioRef} hidden controls>
                <source src={`data:audio/mpeg;base64,${audioSrc}`} type="audio/mpeg" />
              </audio>
            </div>
             )}
          </div>
          ...
          <div onClick={handleDeleteData} className='icon-trash-audio'><FontAwesomeIcon icon={faTrash} /></div>
        </div>
        <div className='container-boxs-audio'>
          <div className='box-left-audio'>
            <div className='upload-box-audio'>
              ...
              {(file ? null : <label htmlFor="file" onClick={() => fileInputRef.current.click()} className='upload-label-audio'>UPLOAD<FontAwesomeIcon icon={faCloudUploadAlt} className='icon-audio' /></label>)}
            </div>
           
            <div className='box-left-data-audio'>
                <div>
                  {original_text && <div className="container-text-audio">{original_text}</div>}
                </div>
            </div>
          </div>
          <div className='box-right-audio'>
            <div className="box-right-data">
              {translatedText  && <div className="text-audio-right">{translatedText}</div>}
            </div>
          </div>
        </div>
      </div>
